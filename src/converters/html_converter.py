...
class HTMLConverter:
    ...
    def _process_element(self, element: PageElement) -> Optional[Any]:
        ...
        # Images
            ...
            src = self._string_attr(element, "src")
            alt = self._string_attr(element, "alt")
            caption = self._string_attr(element, "title")
            if src:
        ...
        for child in element.children:
            if isinstance(child, NavigableString):
                ...
                if tag_name in ["strong", "b"]:
                    ...
                    href = self._string_attr(child, "href")
                    if href:
    ...
    def _get_nested_marks(
        self, element: PageElement, marks: Optional[List[str]] = None
    ) -> List[Dict[str, Any]]:
    ...
    def _string_attr(self, element: Tag, attribute: str) -> str:
        """Return a BeautifulSoup attribute as a plain string."""
        value = element.get(attribute, "")
        if isinstance(value, list):
            return " ".join(str(item) for item in value)
        return cast(str, value or "")
