...
class PostHandler:
    ...
    VALID_AUDIENCES = {"everyone", "only_paid", "only_free"}
    ...
    async def create_draft(
        ...
        if not isinstance(result, dict):
            raise ValueError("Draft creation response was not a dictionary")
        return result
    ...
    async def update_draft(
        ...
        result = self.client.put_draft(post_id, **update_data)
        if not isinstance(result, dict):
            raise ValueError("Draft update response was not a dictionary")
        return result
    ...
    async def publish_draft(self, post_id: str) -> Dict[str, Any]:
        ...
        if not isinstance(result, dict):
            raise ValueError("Publish response was not a dictionary")
        return result
    ...
    async def schedule_draft(
        ...
        self._validate_audience(post_audience, "post_audience")
        self._validate_audience(email_audience, "email_audience")

        normalized_schedule = self._normalize_future_timestamp(scheduled_at)
        draft = self.client.get_draft(post_id)

        if not isinstance(draft, dict):
            raise ValueError("Unable to load draft before scheduling")
        if draft.get("is_published") or draft.get("post_date"):
            raise ValueError("Only unpublished drafts can be scheduled")

        result = self.client.schedule_draft(
            post_id=post_id,
            trigger_at=normalized_schedule,
            post_audience=post_audience,
            email_audience=email_audience,
        )
        if not isinstance(result, dict):
            raise ValueError("Schedule response was not a dictionary")
        return result

    async def list_scheduled_posts(self, limit: int = 10) -> List[Dict[str, Any]]:
        """List drafts that currently have a scheduled release."""
        if not isinstance(limit, int):
            raise ValueError("limit must be an integer")
        if limit < 1 or limit > 25:
            raise ValueError("limit must be between 1 and 25")

        managed_posts = self._get_post_management_rows(
            view="scheduled",
            limit=limit,
            order_by="trigger_at",
            order_direction="asc",
        )
        if managed_posts is not None:
            return [
                {
                    "id": str(post.get("id")),
                    "title": post.get("draft_title") or post.get("title") or "Untitled",
                    "scheduled_at": post.get("trigger_at"),
                    "post_audience": post.get("post_audience") or post.get("audience"),
                    "email_audience": post.get("email_audience")
                    or post.get("audience"),
                    "schedule_count": 1,
                    "schedules": [
                        {
                            "trigger_at": post.get("trigger_at"),
                            "post_audience": post.get("post_audience")
                            or post.get("audience"),
                            "email_audience": post.get("email_audience")
                            or post.get("audience"),
                        }
                ...
                for post in managed_posts[:limit]
            ]

        drafts = await self.list_drafts(limit=25)
        scheduled_posts = []
        ...
        for draft in drafts:
            ...
            next_schedule = min(
                schedules,
                key=lambda item: item.get("trigger_at") or "",
            )
            scheduled_posts.append(
                {
                    ...
                    or draft.get("title")
                    or "Untitled",
                    "scheduled_at": next_schedule.get("trigger_at"),
                    "post_audience": next_schedule.get("post_audience"),
                    "email_audience": next_schedule.get("email_audience"),
                    "schedule_count": len(schedules),
                    "schedules": schedules,
                }
            )

        scheduled_posts.sort(key=lambda item: item.get("scheduled_at") or "")
        return scheduled_posts[:limit]

    def _normalize_future_timestamp(self, value: str) -> str:
        """Normalize an ISO timestamp to the UTC format Substack expects."""
        ...
        try:
            ...
            raise ValueError(
                "scheduled_at must be a valid ISO 8601 datetime, for example 2026-03-10T09:00:00Z"
            ) from exc
        ...
        skew_cutoff = datetime.now(timezone.utc) - timedelta(seconds=60)
        if parsed_utc < skew_cutoff:
            raise ValueError(
                "scheduled_at must be in the future or within 60 seconds of the current time"
            )

        return parsed_utc.replace(microsecond=0).isoformat().replace("+00:00", "Z")

    def _validate_audience(self, value: str, field_name: str) -> None:
        """Validate supported Substack audience enums."""
        if value not in self.VALID_AUDIENCES:
            allowed = ", ".join(sorted(self.VALID_AUDIENCES))
            raise ValueError(f"{field_name} must be one of: {allowed}")

    def _get_post_management_rows(
        self,
        view: str,
        limit: int,
        order_by: str,
        ...
        """Best-effort wrapper around Substack's publish dashboard feed."""
        try:
            result = self.client.get_post_management(
                view=view,
                limit=min(limit, 25),
                offset=0,
                order_by=order_by,
    ...
    async def list_drafts(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        if managed_posts is not None:
            return managed_posts

        try:
            raw_result = self.client.get_drafts(limit=min(limit, 25))
            drafts = list(raw_result)
            logger.info(f"Fallback draft fetch returned {len(drafts)} drafts")
            return drafts[:limit]
    ...
    async def list_published(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        published = [post for post in all_posts if post.get("post_date")]
        return published[:limit]

    async def get_post_analytics(self, post_id: str) -> Dict[str, Any]:
        """Get analytics for a published post from the publish dashboard feed."""
        if not post_id or not isinstance(post_id, str):
            raise ValueError("post_id must be a non-empty string")
        ...
        while True:
            ...
            for post in posts:
                ...
                sent = stats.get("delivered", stats.get("sent", 0)) or 0
                opened = stats.get("opened", 0) or 0
                open_rate = (opened / sent) if sent else None
                return {
                    "id": str(post.get("id")),
                    "title": post.get("title") or "Untitled",
                    "post_date": post.get("post_date"),
                    "email_sent_at": post.get("email_sent_at"),
                    "views": stats.get("views", 0) or 0,
                    "sent": stats.get("sent", 0) or 0,
                    "delivered": stats.get("delivered", 0) or 0,
                    "opened": opened,
                    "open_rate": open_rate,
                    "signups": stats.get("signups", 0) or 0,
                    "subscribes": stats.get("subscribes", 0) or 0,
                    "estimated_value": stats.get("estimated_value"),
        ...
        raise ValueError(
            f"Could not find published post {post_id} in the analytics feed"
        )
    ...
    async def get_post(self, post_id: str) -> Dict[str, Any]:
        ...
        if not isinstance(result, dict):
            raise ValueError("Post lookup response was not a dictionary")
        return result
    ...
    async def duplicate_post(
        ...
        try:
            ...
            result = self.client.post_draft(post.get_draft())
            if not isinstance(result, dict):
                raise ValueError("Duplicate draft response was not a dictionary")
            return result
    ...
    def _add_blocks_to_post(self, post: Post, blocks: List[Dict[str, Any]]) -> None:
        ...
        for block in blocks:
            ...
            if block_type == "paragraph":
                post.draft_body["content"].append(self._build_paragraph_node(content))
                ...
                post.draft_body["content"].append(
                    {
                        "type": "heading",
                        "attrs": {"level": actual_level},
                        "content": self._normalize_inline_content(content),
                    }
                )

            elif block_type == "hr":
                post.draft_body["content"].append({"type": "horizontal_rule"})

            elif block_type == "paywall":
                post.draft_body["content"].append({"type": "paywall"})

            elif block_type == "captioned-image":
                post.draft_body["content"].append(self._build_image_node(block))
                ...
                post.draft_body["content"].append(code_block)
                ...
                for i, item in enumerate(list_items, 1):
                    if isinstance(item, dict) and "content" in item:
                        ...
                        if isinstance(item_content, list) and len(item_content) > 0:
                            normalized = self._normalize_inline_content(
                                item_content[0].get("content", [])
                            )
                            prefix = (
                                "\u2022 " if block_type == "bulleted-list" else f"{i}. "
                            )
                            post.draft_body["content"].append(
                                {
                                    "type": "paragraph",
                ...
                post.draft_body["content"].append(
                    {
                        "type": "blockquote",
                        "content": [self._build_paragraph_node(content)],
                    }
                )
                ...
                if content:
                    post.draft_body["content"].append(
                        self._build_paragraph_node(content)
                    )

    def _build_paragraph_node(self, content: Any) -> Dict[str, Any]:
        """Build a paragraph node for the serialized draft body."""
        return {
            "type": "paragraph",
            "content": self._normalize_inline_content(content),
        }

    def _normalize_inline_content(self, content: Any) -> List[Dict[str, Any]]:
        """Convert converter text nodes into the structure expected by draft_body."""
        ...
        for item in content or []:
            ...
            if item_type == "text":
                ...
                marks = self._normalize_marks(item.get("marks", []))
                if marks:
                    node["marks"] = marks
                normalized.append(node)
            elif item_type == "paragraph":
                normalized.extend(
                    self._normalize_inline_content(item.get("content", []))
                )
            else:
                nested_content = item.get("content")
                if nested_content:
                    normalized.extend(self._normalize_inline_content(nested_content))

        return normalized or [{"type": "text", "text": ""}]

    def _normalize_marks(self, marks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Normalize text marks to the shape used by python-substack draft JSON."""
        ...
        for mark in marks or []:
            ...
            normalized_mark: Dict[str, Any] = {"type": mark_type}
            if mark_type == "link":
                href = mark.get("href")
                if not href and isinstance(mark.get("attrs"), dict):
                    href = mark["attrs"].get("href")
                if href:
                    normalized_mark["attrs"] = {"href": href}
            normalized_marks.append(normalized_mark)
        return normalized_marks

    def _build_image_node(self, block: Dict[str, Any]) -> Dict[str, Any]:
        """Build the captioned-image node shape expected by Substack drafts."""
        return {
            "type": "captionedImage",
            "content": [
                {
                    ...
                    "attrs": {
                        ...
                        "fullscreen": False,
                        "imageSize": "normal",
                        "height": block.get("height", 819),
                        "width": block.get("width", 1456),
                        "resizeWidth": block.get("resizeWidth", 728),
    ...
    def _add_formatted_content_to_paragraph(self, para: Any, content: Any) -> None:
    ...
    def _convert_content_to_text_nodes(self, content: Any) -> List[Dict[str, Any]]:
