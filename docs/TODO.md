...
#### 6. Schedule Post Release Verification 🟡
**Risk**: Scheduling is back, but still depends on a reverse-engineered private endpoint
**Impact**: Feature may regress across publications or after upstream changes
**Fix**: Keep `schedule_post` behind real live-smoke validation before release calls
**Tool**: `schedule_post`
**Follow-up**: Run the live account smoke suite and document real-account results
...
### 3. Improve Test Coverage Beyond 72% 🟡
**Goal**: Increase coverage from 72% toward 80%+ with focus on release-critical paths

**Subtasks**:
- [ ] Raise `src/server.py` coverage above 50%
- [ ] Raise `src/utils/api_wrapper.py` above 80%
- [ ] Add more high-signal `post_handler` branch coverage
- [ ] Expand live authenticated smoke coverage
