...
### 9. Schedule Post Is Best-Effort
- **Issue**: `schedule_post` is implemented through a reverse-engineered private endpoint whose availability can vary by account or by Substack changes
- **Impact**: Scheduling may work in one environment and fail with `404` or other upstream errors in another
- **Cause**: The server uses a custom `scheduled_release` endpoint rather than a stable official API
- **Workaround**: Run the live smoke suite in [LIVE_SMOKE_TEST.md](LIVE_SMOKE_TEST.md) before treating scheduling as release-ready for your publication
- **Status**: Tool is exposed again, but still requires live verification on real credentials
...
### 11. Live Auth Coverage Is Opt-In
- **Issue**: Most local test runs skip credentialed Substack checks
- **Impact**: A green default suite does not automatically prove live account behavior
- **Cause**: `requires_auth` and `requires_auth_write` tests are intentionally gated on environment credentials
- **Workaround**: Run the live smoke instructions in [LIVE_SMOKE_TEST.md](LIVE_SMOKE_TEST.md)
- **Status**: Expected repository behavior, but it must be part of release audits
