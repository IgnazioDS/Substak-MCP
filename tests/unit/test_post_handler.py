...
This is for paid subscribers only."""
    ...
    @pytest.mark.asyncio
    async def test_create_draft_preserves_inline_marks_in_draft_body(self):
        """Draft serialization should preserve rich-text marks instead of markdown literals."""
        self.mock_client.post_draft = Mock(return_value={"id": "post-123"})
        ...
        draft_data = self.mock_client.post_draft.call_args[0][0]
        draft_body = json.loads(draft_data["draft_body"])
        paragraph = draft_body["content"][0]
        text_nodes = paragraph["content"]

        assert paragraph["type"] == "paragraph"
        assert any(
            node.get("text") == "bold" and {"type": "strong"} in node.get("marks", [])
            for node in text_nodes
        )
        assert any(
            node.get("text") == "italic" and {"type": "em"} in node.get("marks", [])
            for node in text_nodes
        )
        assert any(
            node.get("text") == "code" and {"type": "code"} in node.get("marks", [])
            for node in text_nodes
        )
        assert any(
            node.get("text") == "a link"
            and {
                "type": "link",
    ...
    @pytest.mark.asyncio
    async def test_create_draft_preserves_image_and_blockquote_structure(self):
        """Images and blockquotes should serialize as structured nodes, not markdown text."""
        self.mock_client.post_draft = Mock(return_value={"id": "post-123"})
        ...
        blockquote = draft_body["content"][0]
        image_block = draft_body["content"][1]

        assert blockquote["type"] == "blockquote"
        assert blockquote["content"][0]["type"] == "paragraph"
        assert blockquote["content"][0]["content"][0]["text"] == "Quoted line"

        assert image_block["type"] == "captionedImage"
        assert image_block["content"][0]["type"] == "image2"
        assert (
            image_block["content"][0]["attrs"]["src"] == "https://example.com/image.jpg"
        )
        assert image_block["content"][0]["attrs"]["alt"] == "Alt text"
        assert image_block["content"][0]["attrs"]["title"] == "Caption"
