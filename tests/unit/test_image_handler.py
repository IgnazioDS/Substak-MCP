...
class TestImageHandler:
    ...
    async def test_upload_image_from_file_path(self):
        ...
        # Mock file reading
        mock_image_data = self.PNG_HEADER

        with patch("os.path.exists", return_value=True):
            with patch("os.path.getsize", return_value=len(mock_image_data)):
                with patch(
                    "builtins.open", MagicMock(return_value=io.BytesIO(mock_image_data))
                ):
                    # Mock the get_image response
                    self.mock_client.get_image = Mock(
                        return_value={
                ...
                assert (
                    result["url"]
                    == "https://substackcdn.com/image/fetch/w_1456,c_limit,f_webp,q_auto:good,fl_progressive:steep/https://example.com/uploaded.jpg"
                )
    ...
    async def test_upload_image_from_url(self):
        ...
        with patch.object(
            self.handler,
            "_inspect_remote_image",
            AsyncMock(return_value=("image/jpeg", 12345)),
        ):
            ...
            with patch("os.chmod"):
                with patch("os.fdopen", return_value=io.BytesIO()):
                    with patch("os.unlink"):
                        result = await self.handler.upload_image(
                            image_data, filename="custom.jpg"
                        )
    ...
    async def test_validate_image_format(self):
        ...
        assert (
            self.handler._validate_image_format("IMAGE.JPG") is True
        )  # Case insensitive
    ...
    @pytest.mark.asyncio
    async def test_upload_rejects_oversized_local_file(self):
        """Local files larger than 25 MB should be rejected before upload."""
        with patch("os.path.exists", return_value=True):
            with patch("os.path.getsize", return_value=(25 * 1024 * 1024) + 1):
                with pytest.raises(ValueError, match="25 MB"):
                    await self.handler.upload_image("/path/to/image.jpg")

    @pytest.mark.asyncio
    async def test_upload_rejects_non_image_content_with_image_extension(self):
        """Local files must pass content sniffing, not just extension checks."""
        with patch("os.path.exists", return_value=True):
            with patch("os.path.getsize", return_value=1024):
                with patch(
                    "builtins.open", MagicMock(return_value=io.BytesIO(b"not an image"))
                ):
                    with pytest.raises(ValueError, match="Unsupported image content"):
                        await self.handler.upload_image("/path/to/image.jpg")

    @pytest.mark.asyncio
    async def test_upload_rejects_oversized_bytes(self):
        """Byte uploads larger than 25 MB should be rejected before temp file creation."""
        with pytest.raises(ValueError, match="25 MB"):
            await self.handler.upload_image(b"x" * ((25 * 1024 * 1024) + 1), "big.jpg")
    ...
    async def test_upload_url_fetch_error(self):
        ...
        self.mock_client.get_image = Mock(
            side_effect=Exception("Failed to fetch image")
        )
    ...
    async def test_upload_api_error(self):
        ...
        with patch("os.path.exists", return_value=True):
            with patch("os.path.getsize", return_value=len(self.PNG_HEADER)):
                with patch(
                    "builtins.open", MagicMock(return_value=io.BytesIO(self.PNG_HEADER))
                ):
    ...
    def test_get_optimized_url(self):
        ...
        low_quality_url = self.handler.get_optimized_url(
            original_url, quality="auto:low"
        )
    ...
    async def test_batch_upload(self):
        ...
        with patch("os.path.exists", return_value=True):
            with patch("os.path.getsize", return_value=len(self.PNG_HEADER)):
                ...
                mock_files = [io.BytesIO(self.PNG_HEADER) for _ in range(3)]
                ...
                with patch("builtins.open", mock_open):
                    self.mock_client.get_image = Mock(
                        side_effect=[
                            {
                                "url": f"https://substackcdn.com/image{i}.jpg",
                                "id": f"image-{i}",
                                "contentType": "image/jpeg",
                                "bytes": 12345,
                                "imageWidth": 800,
