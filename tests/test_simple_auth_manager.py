...
class TestSimpleAuthManager:
    ...
    def test_store_and_retrieve_token(self, auth_manager):
        ...
        # Store a token
        test_token = "dummy-session-value"
    ...
    def test_refuses_to_load_cookie_jar_without_substack_sid(self, auth_manager):
        """Stored cookies without the session cookie should be rejected."""
        auth_manager.store_session_cookies(
            {"substack.sid": "dummy-session-value", "substack.lli": "login-state"},
            "test@example.com",
        )
        ...
        encrypted_cookies = base64.b64decode(auth_data["cookies"].encode())
        auth_data["cookies"] = base64.b64encode(
            auth_manager.cipher.encrypt(
                json.dumps({"substack.lli": "login-state"}).encode()
            )
        ).decode()
        auth_manager.auth_file.write_text(json.dumps(auth_data))

        assert auth_manager.get_session_cookies() is None
        assert auth_manager.get_token() == "dummy-session-value"

    def test_refuses_to_load_when_key_file_is_missing(self, auth_manager):
        """Existing auth should not be readable after key loss."""
        auth_manager.store_session_cookies(
            {"substack.sid": "dummy-session-value", "substack.lli": "login-state"},
            "test@example.com",
        )

        auth_manager.key_file.unlink()

        with patch(
            "src.simple_auth_manager.Path.home",
            return_value=auth_manager.config_dir.parent,
        ):
            broken_manager = SimpleAuthManager("https://test.substack.com")

        assert broken_manager.get_token() is None
        assert broken_manager.get_session_cookies() is None
